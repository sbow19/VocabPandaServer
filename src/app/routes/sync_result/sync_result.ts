...
usersRouter.post("/", async(req: apiTypes.LocalSyncResponse, res: apiTypes.Acknowledgement)=>{

    const syncResultObject: apiTypes.LocalBackendSyncResult = req.body;
    ...
    try{
        ...
        let acknowledgement: apiTypes.BEAcknowledgement = {
            requestId: deviceId,
            userId: syncResultObject.userId,
        ...
        if(syncResultObject.userContentSync.valid){
            ...
            const syncFlagResult = await UserSyncDatabase.checkSyncFlags(
                syncResultObject.userId,
                [syncResultObject.requestId],
                deviceId
            )
        ...
        if(e.fullSyncRequired){
            ...
            const prepareFullSyncResponse = await SyncProcess.prepareFullSyncContent(
                syncResultObject.userId,
                syncResultObject.requestId,
                deviceId,
                syncResultObject.deviceType
            );
